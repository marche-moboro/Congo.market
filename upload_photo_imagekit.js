...
async function uploadPhoto(file, folder = 'sellers') {
  ...
  try {
    ...
    // ✅ Condition corrigée : !data.url suffit (data.message existe aussi en succès)
    if (!data.url) {
...
async function cleanOrphanImages() {
  try {
    ...
    products?.forEach(p => { if (p.image) activeUrls.add(p.image); });
    promos?.forEach(p   => { if (p.image) activeUrls.add(p.image); });
    sellers?.forEach(s  => { if (s.photo) activeUrls.add(s.photo); });
